...
class intensidades:
    ...
    def parameters(self, nombre_I_0:str = None, nombre_I_90:str = None, nombre_I_45:str = None, nombre_I_135:str = None, nombre_I_RHC:str = None, nombre_I_LHC:str = None):
        ...
        self.imagenI_0 = np.array(Image.open(nombre_I_0).convert('L'))

        self.imagenI_90 = np.array(Image.open(nombre_I_90).convert('L'))
        
        self.imagenI_45 = np.array(Image.open(nombre_I_45).convert('L'))
        
        self.imagenI_135 = np.array(Image.open(nombre_I_135).convert('L'))

        self.imagen_RHC = np.array(Image.open(nombre_I_RHC).convert('L'))

        self.imagen_LHC = np.array(Image.open(nombre_I_LHC).convert('L'))
        

        self.s_0 = np.sum(self.imagenI_0) + np.sum(self.imagenI_90)
        self.s_1 = (np.sum(self.imagenI_0) - np.sum(self.imagenI_90))/self.s_0
        self.s_2 = np.sum(self.imagenI_45) - np.sum(self.imagenI_135)/self.s_0
